...
import { compareRegimes } from '@/lib/calculations/tax-optimization';
...
export async function GET(
  ...
  try {
    ...
    const regimeComparisons = compareRegimes(demoProperties, demoTransactions, 2024);

    return NextResponse.json({
      success: true,
      regime_comparisons: regimeComparisons,
      metadata: {
        properties_count: demoProperties.length,
        total_portfolio_value: demoProperties.reduce((sum, p) => sum + p.current_value, 0),
        total_annual_income: demoTransactions
    ...
    console.error('Error generating regime comparison:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to generate regime comparison' },
      { status: 500 }
    );
  }
}